...
joint_v0_vw = cj.newDist([-80.0,cj.base(c_vw)[2]],[40.0,cj.size(c_vw)[2]],[100,100], [a for a in np.zeros(10000)])

cj.joint2D(v0, 0, vw_given_v, joint_v0_vw)

joint_v0_vu = cj.newDist([-80.0,cj.base(c_vu)[2]],[40.0,cj.size(c_vu)[2]],[100,100], [a for a in np.zeros(10000)])
...
marginal_vw_vwvu = cj.newDist([cj.base(c_vw)[2],cj.base(c_vwvu)[2]],[cj.size(c_vw)[2],cj.size(c_vwvu)[2]],[100,100], [a for a in np.zeros(10000)])
marginal_vw = cj.newDist([cj.base(c_vw)[2]],[cj.size(c_vw)[2]],[100], [a for a in np.zeros(100)])
...
vwvu_given_vu = cj.newDist([cj.base(c_vu)[2],cj.base(c_vwvu)[2]],[cj.size(c_vu)[2],cj.size(c_vwvu)[2]],[100,100], [a for a in np.zeros(10000)])

cj.conditional(marginal_vu_vwvu, [0], marginal_vu, vwvu_given_vu)

joint_v0_vw_vwvu = cj.newDist([-80.0,cj.base(c_vw)[2],cj.base(c_vwvu)[2]],[40.0,cj.size(c_vw)[2],cj.size(c_vwvu)[2]],[100,100,100], [a for a in np.zeros(1000000)])
...
marginal_vwvu_v1 = cj.newDist([cj.base(c_vwvu)[2],-80.0],[cj.size(c_vwvu)[2],40.0],[100,100], [a for a in np.zeros(10000)])
...
v1_given_w0 = cj.newDist([-1.0, -80.0], [26.0, 40.0], [100,100], [a for a in np.zeros(10000)])
v1_given_u0 = cj.newDist([-5.0, -80.0], [55.0, 40.0], [100,100], [a for a in np.zeros(10000)])
...
joint_w0_vw_vwvu = cj.newDist([-1.0,cj.base(c_vw)[2],cj.base(c_vwvu)[2]],[26.0,cj.size(c_vw)[2],cj.size(c_vwvu)[2]],[100,100,100], [a for a in np.zeros(1000000)])
marginal_w0_vw = cj.newDist([-1.0,cj.base(c_vw)[2]],[26.0,cj.size(c_vw)[2]],[100,100], [a for a in np.zeros(10000)])
...
marginal_w0_vwvu = cj.newDist([-1.0,cj.base(c_vwvu)[2]],[26.0,cj.size(c_vwvu)[2]],[100,100], [a for a in np.zeros(10000)])

cj.marginal(joint_w0_vw_vwvu, 1, marginal_w0_vwvu)

# Now get v1|vwvu. We already have marginal_vwvu_v1

marginal_vwvu = cj.newDist([cj.base(c_vwvu)[2]],[cj.size(c_vwvu)[2]],[100], [a for a in np.zeros(100)])

cj.marginal(marginal_vwvu_v1, 1, marginal_vwvu)

v1_given_vwvu = cj.newDist([-80.0,cj.base(c_vwvu)[2]],[40.0,cj.size(c_vwvu)[2]],[100,100], [a for a in np.zeros(10000)])
...
joint_w0_vwvu_v1 = cj.newDist([-1.0,cj.base(c_vwvu)[2],-80.0],[26.0,cj.size(c_vwvu)[2],40.0],[100,100,100], [a for a in np.zeros(1000000)])
marginal_w0_v1 = cj.newDist([-1.0, -80.0], [26.0, 40.0], [100,100], [a for a in np.zeros(10000)])
...
joint_u0_vu_vwvu = cj.newDist([-5.0,cj.base(c_vu)[2],cj.base(c_vwvu)[2]],[55.0,cj.size(c_vu)[2],cj.size(c_vwvu)[2]],[100,100,100], [a for a in np.zeros(1000000)])
marginal_u0_vu = cj.newDist([-5.0,cj.base(c_vu)[2]],[55.0,cj.size(c_vu)[2]],[100,100], [a for a in np.zeros(10000)])
...
marginal_u0_vwvu = cj.newDist([-5.0,cj.base(c_vwvu)[2]],[55.0,cj.size(c_vwvu)[2]],[100,100], [a for a in np.zeros(10000)])

cj.marginal(joint_u0_vu_vwvu, 1, marginal_u0_vwvu)

# Finally, calculate the joint w0,vwvu,v1 then find the marginal

joint_u0_vwvu_v1 = cj.newDist([-5.0,cj.base(c_vwvu)[2],-80.0],[55.0,cj.size(c_vwvu)[2],40.0],[100,100,100], [a for a in np.zeros(1000000)])
marginal_u0_v1 = cj.newDist([-5.0, -80.0], [55.0, 40.0], [100,100], [a for a in np.zeros(10000)])
...
v2 = cj.newDist([-80.0],[40.0],[100],[a for a in np.zeros(100)])
w2 = cj.newDist([-1.0],[26.0],[100],[a for a in np.zeros(100)])
u2 = cj.newDist([-5.0],[55.0],[100],[a for a in np.zeros(100)])

# Lets try 10 iterations
for i in range(100):
    # Update w' and u' : easy, just chained and independent of everything else
    # Reuse w0w1 and u0u1
    cj.joint2D(w1, 0, c_w_prime, w0w1)
    cj.joint2D(u1, 0, c_u_prime, u0u1)
