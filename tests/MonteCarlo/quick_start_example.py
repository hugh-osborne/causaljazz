...
from causaljazz.inference import TEDAG_MC
...
func_e_x2, func_x2_noise = data.trainANNForMC('x2_given_x1', generate_models, data_points, [input_res], output_res)
...
func_e_x3, func_x3_noise = data.trainANNForMC('x3_given_x1x2c', generate_models, data_points, [input_res,input_res,input_res], output_res)

# S <- X1,X2,C,X3
data_points = np.stack([ground[:,0], ground[:,1], generated_c, ground[:,3], ground[:,4]])
func_e_s, func_s_noise  = data.trainANNForMC('s_given_x1x2cx3', generate_models, data_points, [input_res,input_res,input_res,input_res], output_res)
...
tedag_func_x2_e = TEDAG_MC.FUNCTION(['X1'], 'X2E', 0, func_e_x2)
tedag_func_x2_noise = TEDAG_MC.FUNCTION(['X1'], 'X2N', 0, func_x2_noise)
tedag_func_x2 = TEDAG_MC.FUNCTION(['X2E', 'X2N'], 'X2', 0, func_sum)
tedag_func_lc_e = TEDAG_MC.FUNCTION(['X1', 'X2'], 'LCE', 0, func_c_exp)
tedag_func_lc_noise = TEDAG_MC.FUNCTION(['X1', 'X2'], 'LCN', 0, func_c_noise)
tedag_func_lc = TEDAG_MC.FUNCTION(['LCE', 'LCN'], 'LC', 0, func_sum)
tedag_func_x3_e = TEDAG_MC.FUNCTION(['X1', 'X2', 'LC'], 'X3E', 0, func_e_x3)
tedag_func_x3_noise = TEDAG_MC.FUNCTION(['X1', 'X2', 'LC'], 'X3N', 0, func_x3_noise)
tedag_func_x3 = TEDAG_MC.FUNCTION(['X3E', 'X3N'], 'X3', 0, func_sum)
tedag_func_s_e  = TEDAG_MC.FUNCTION(['X1', 'X2', 'LC', 'X3'], 'SE', 0, func_e_s)
tedag_func_s_noise  = TEDAG_MC.FUNCTION(['X1', 'X2', 'LC', 'X3'], 'SN', 0, func_s_noise)
tedag_func_s = TEDAG_MC.FUNCTION(['SE', 'SN'], 'S', 0, func_sum)

# Initialise the TEDAG
tedag = TEDAG_MC(1, [tedag_func_x2_e,tedag_func_x2_noise,tedag_func_lc_e,tedag_func_lc_noise,tedag_func_x2,tedag_func_lc,tedag_func_x3_e,tedag_func_x3_noise,tedag_func_x3,tedag_func_s_e,tedag_func_s_noise,tedag_func_s], observables=['X1', 'X2', 'LC', 'X3', 'S'], verbose=True)
